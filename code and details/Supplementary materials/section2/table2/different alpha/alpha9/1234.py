...
# for i in 10; do python ./alpha$i/1234.py --step4 --alpha $i --l1depth 10 --l1node 50 --l2depth 6 --l2node 100 >> ./alpha$i/result4.txt &;done
...
class DeepHPM:
    ...
    def idn_net_f(self, t, yt):
        ...
        zeros = tf.constant(0.0, shape=[36001,1])
        ttt =  tf.constant(self.alpha_value, shape=[36001,1])
    ...
    #step3 loss_f函数 （这和step2 loss_f函数一致）--为了区分就写成2部分，但是修改参数的时候，就需要分别修改   
    def sol_net_f(self, t, yt):
        u, u_t = self.idn_net_u(t)
        # u, u_t = self.idn_net_u(t, yt)
        zeros = tf.constant(0.0,shape=[36001,1])
        ttt =  tf.constant(0.1,shape=[36001,1])
        para = tf.concat([ttt, zeros, zeros, zeros, zeros, zeros], 1)
        yt_array = tf.concat([yt, zeros, zeros, zeros, zeros, zeros], 1)
        yt_array_ = tf.multiply(para, yt_array)
        f =  u_t - (self.net_pde(u)+yt_array_)
...
if __name__ == "__main__":
    ...
    # checkpoint_file_step1 = "./test_1_1/test_1_1.ckpt"
    # checkpoint_file_step2 = "./test_1_2/test_1_2.ckpt"
    # checkpoint_file_step3 = "./test_1_3/test_1_3.ckpt"
    ...
    if args.step1 :
        ...
        # With U(x,t) to train Right hand side f(U,Ux,Uxx)
        ...
        for m in [100, 1000, 2400]:
            ...
            for y in [y_1, y_2, y_3, y_4, y_5, y_6]:
                ...
                plt.plot(x, all_together[:,j-1],'--*r',label='prediction')  
                plt.legend(['true','prediction'])
                plt.savefig(f'./alpha%d/pred_%d_%d.jpg'%(args.alpha, m, j)) 
                j = j+1
